    ...
    try {
      ...
      setMessages(prev => {
          ...
          return [...filtered, {
            id: (Date.now() + 1).toString(),
            role: Role.SYSTEM,
            content: "Erreur technique. Veuillez reformuler ou rafraîchir la page.",
            timestamp: Date.now()
          }];
      });
    } finally {
    ...
    <div className="flex flex-col h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-4 py-2 flex items-center justify-between shadow-sm no-print">
        <div className="flex items-center gap-2">
           <div className="w-8 h-8 bg-legal-900 rounded-lg flex items-center justify-center text-white shadow-md">
           ...
           <div>
             <h1 className="text-sm font-bold text-legal-900 font-serif-legal leading-tight">JuristeAI</h1>
             <p className="text-[7px] uppercase tracking-widest text-gold-600 font-black">Assistant JurisprudencesPro</p>
           </div>
        </div>
        
        <div className="flex items-center gap-3">
          <div className="hidden xs:flex gap-1">
            <span className="text-[8px] font-bold px-1 py-0.5 bg-slate-100 rounded text-slate-500">MA/FR/EG</span>
          </div>
          <button onClick={clearChat} className="text-slate-300 hover:text-red-500 transition-colors p-1" title="Réinitialiser">
